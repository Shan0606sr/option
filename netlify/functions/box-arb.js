...
const { boxArbUniverse, applyBoxSeedBooks } = require("./lib/box-arb");
...
exports.handler = async (event) => {
  ...
  try {
    const snapshot = await boxArbUniverse(access, {
    ...
    if (keys.length) {
      ...
      extraBooks = applyBoxSeedBooks(
        snapshot.pairs.filter((row) => (
          keys.includes(row.k1_ce_key) || keys.includes(row.k1_pe_key)
          || keys.includes(row.k2_ce_key) || keys.includes(row.k2_pe_key)
        )),
        quoted.books || {},
      );
    ...
    return json(200, {
      ...
      message: snapshot.pairs.length
        ? `Box universe: ${snapshot.stocks_scanned} names, ${snapshot.pair_count} K1/K2 strike pairs.`
        : (snapshot.price_error || "No matching same-expiry strike pairs."),
      ...snapshot,
