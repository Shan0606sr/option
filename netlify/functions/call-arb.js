...
const { callArbUniverse, applySeedBooks } = require("./lib/call-arb");
...
exports.handler = async (event) => {
  ...
  try {
    const snapshot = await callArbUniverse(access, {
    ...
    if (keys.length) {
      ...
      extraBooks = applySeedBooks(
        snapshot.pairs.filter((row) => keys.includes(row.ce_key) || keys.includes(row.pe_key) || keys.includes(row.fut_key)),
        quoted.books || {},
      );
    ...
    return json(200, {
      ...
      message: snapshot.pairs.length
        ? `Stock call-arb universe: ${snapshot.stocks_scanned} names, ${snapshot.pair_count} same-expiry CE/PE/FUT sets.`
        : (snapshot.price_error || "No matching stock CE/PE/Future sets."),
      ...snapshot,
