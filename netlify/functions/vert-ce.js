...
const { vertCeUniverse, applyVertCeSeedBooks } = require("./lib/vert-ce");
...
exports.handler = async (event) => {
  ...
  try {
    const snapshot = await vertCeUniverse(access, {
    ...
    if (keys.length) {
      ...
      extraBooks = applyVertCeSeedBooks(
        snapshot.pairs.filter((row) => keys.includes(row.k1_ce_key) || keys.includes(row.k2_ce_key)),
        quoted.books || {},
      );
    ...
    return json(200, {
      ...
      message: snapshot.pairs.length
        ? `Call vertical universe: ${snapshot.stocks_scanned} names, ${snapshot.pair_count} K1/K2 Call pairs.`
        : (snapshot.price_error || "No matching same-expiry Call pairs."),
      ...snapshot,
