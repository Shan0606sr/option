...
const { butterflyArbUniverse, applyButterflySeedBooks } = require("./lib/butterfly-arb");
...
exports.handler = async (event) => {
  ...
  try {
    const snapshot = await butterflyArbUniverse(access, {
    ...
    if (keys.length) {
      ...
      extraBooks = applyButterflySeedBooks(
        snapshot.pairs.filter((row) => keys.includes(row.k1_ce_key) || keys.includes(row.k2_ce_key) || keys.includes(row.k3_ce_key)
          || keys.includes(row.k1_pe_key) || keys.includes(row.k2_pe_key) || keys.includes(row.k3_pe_key)),
        quoted.books || {},
      );
    ...
    return json(200, {
      ...
      message: snapshot.pairs.length
        ? `Butterfly universe: ${snapshot.stocks_scanned} names, ${snapshot.pair_count} equidistant K1/K2/K3 sets.`
        : (snapshot.price_error || "No equidistant Call/Put butterflies."),
      ...snapshot,
